...
export class Viseur {
    ...
    private updateCurrentTurn(turn: number): void {
        ...
        while (turn < nextTurn && d.indexOfNextTurn > 0) {
            const r = deltas[d.indexOfNextTurn] && deltas[d.indexOfNextTurn].reversed;

            if (r) {
                d.nextState = this.parser.mergeDelta(d.nextState, r);
        ...
        while (nextTurn < turn + 1) {
            ...
            if (deltas[d.indexOfNextTurn]) {
                d.nextState = this.parser.mergeDelta(d.nextState, deltas[d.indexOfNextTurn].game);
                nextTurn = this.getTurnNumber(d.nextState);
            }
        }
